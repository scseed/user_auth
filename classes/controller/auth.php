...
class Controller_Auth extends Controller_Template {
	...
	public function before()
	{
		...
		if($this->request->action() == 'login_panel')
		{
			...
			{
				$this->request->action('user_panel');
			}
		}
	...
	public function action_hash_login()
	{
		...
		else
		{
			...
			$this->request->redirect(Route::url('auth', array('action' => 'change_password', 'lang' => I18n::lang())));
		}
	}
	...
	public function action_login_panel()
	{
		...
		$errors = NULL;
		if($_POST
		   AND Request::initial()->action() != 'pass_remind'
		   AND Request::initial()->action() != 'change_password'
		   AND Request::initial()->action() != 'registration'
		   AND Request::initial()->action() != 'login')
		{
			$post = Arr::extract($_POST, array('email', 'password', 'remember'));
			$post['remember'] = TRUE;
	...
	public function action_registration()
	{
		...
		if(Auth::instance()->logged_in() AND ! $partner_registration)
			$this->request->redirect(Route::url('auth', array('action' => 'user', 'lang' => I18n::lang())));

//		StaticJs::instance()
//			->addJs('/js/jquery.maskedinput.min.js')
//			->addJs('/js/jquery.tooltips.min.js')
//			->addJs('/js/form.js')
//			;
		...
		if($_POST)
		{
			$post = Arr::extract($_POST, array_keys($fields), NULL);
	...
	public function action_pass_remind()
	{
		...
		if($_POST)
		{
			$post = Validation::factory(Arr::extract($_POST, array('email')))
	 ...
	 * @return void
	 */
	public function action_user_panel()
	{
		$user = Auth::instance()->get_user();

		$this->template->content = View::factory('frontend/content/auth/user_panel')
			->bind('user', $user)
		;
	}
	...
	public function action_user()
	{
		...
		$enrollments = Jelly::query('enrollment')
			->where('user', '=', $user->id)
			->where('date_submit', '>', strtotime(date('d.m.Y', time()-7862400)))
			->select()
		;

		$users = NULL;
		$count_prtps = $user->prtps->count();
		if($user->is_partner AND $count_prtps)
		{
			$_events = Jelly::query('event')->active()->select();
			foreach($user->prtps as $prtp)
			{
				foreach($_events as $event)
				{
					$events[$prtp->id][$event->id] = array(
						'id' => $event->id,
						'is_active' => TRUE,
						'name' => $event->name
					);
			...
			$users = View::factory('frontend/content/auth/users')
				->bind('events', $events)
				->bind('count_prtps', $count_prtps)
				->set('users', $user->get('prtps')->order_by('date_create', 'DESC')->select());
		}

		$this->page_title = __('Личный кабинет');
		$this->template->content = View::factory('frontend/content/auth/user')
			->bind('user', $user)
			->bind('users', $users)
...
//	{
//		$user = Jelly::query('user', 179)->select();
//		$password = Text::random();
//		$message_user = View::factory('frontend/template/email')
//			->set('content', View::factory('frontend/content/auth/mail/credentials')
//				->bind('password', $password)
